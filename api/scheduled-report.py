...
Lark tai 3 moc gio co dinh: Trua (12h) / Chieu (15h) / Toi (22h). Duoc trigger
boi Vercel Cron Jobs (xem "crons" trong vercel.json) - chay tren cloud 24/7,
KHONG phu thuoc may Mac bat/tat (thay the launchd local lam kenh chinh, xem
ghi chu trong memory du an ve ly do chuyen).

Nhan dien slot qua query param ?slot=trua|chieu|toi (Vercel Cron goi GET toi
duong dan cau hinh san trong vercel.json, khong doc body). Bao ve bang
CRON_SECRET (Vercel tu dong gui header Authorization: Bearer <CRON_SECRET>
cho cac request tu chinh Cron Job cua no) de tranh bi trigger tuy tien qua
URL cong khai.

QUAN TRONG: tinh doanh thu theo MOC GIO CO DINH (12:00:00/15:00:00/22:00:00),
KHONG phai "gio hien tai luc cron chay" - vi Vercel co the bi delay vai phut,
neu tinh theo gio thuc te se ra so sai (bai hoc tu su co GitHub Actions truoc
day). Neu bi delay qua nua dem thi van tinh cho ngay hom truoc.
"""
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if slot not in SLOT_CUTOFFS:
            self._respond(400, {"ok": False, "error": f"invalid slot: {slot!r}"})
            return
